...
for i in range(burstParams.shape[0]):
    ...
    for j in range(len(np.where(galaxyParams[:,0]==burstParams[i,0]))):    
        NLOD=NLOD+np.load('GalacticNLOD'+str(burstParams[i,0])+str(j)+'.npy')[100:,:]
    NLODTotal=NLODTotal+NLOD
    #plot of lensing probability (1-prob of not lensing)
    fig, ax1 = plt.subplots(1)
    heatmap=ax1.imshow(NLOD.T, cmap='viridis', interpolation='nearest', aspect=len(Mset)/len(fdm)-1.5, origin='lower')
    cbar1=fig.colorbar(heatmap)
    cbar1.set_label(r'$\tau$', fontsize=24)
    ...
    plt.show()
    fig.savefig('GalacticLensOD'+str(burstParams[i,0])+'.pdf')


fig, ax1 = plt.subplots(1)
heatmap=ax1.imshow(NLODTotal.T, cmap='viridis', interpolation='nearest', aspect=len(Mset)/len(fdm)-1.5, origin='lower')
cbar1=fig.colorbar(heatmap)
cbar1.set_label(r'$\tau$', fontsize=24)
ax1.set_ylabel("f$_{DM}$", fontsize=24)
ax1.set_xlabel("Lens Mass (log$_{10}$(M$_\odot$))", fontsize=24)
...
ax1.set_yticklabels(np.around(fdm[ylocs], 2))
plt.show()
fig.savefig('GalacticLensODTotal.pdf')
